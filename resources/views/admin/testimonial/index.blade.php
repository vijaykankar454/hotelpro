...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">{{ __('View Testimonials') }}</h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="#">{{ __('Home') }}</a></li>
          <li class="breadcrumb-item active"> {{ __('View Testimonials') }}</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">View Testimonials</h3>
            <a href='{{route('admin.testimonial.testimonialadd')}}' class="btn btn-sm btn-primary pull-left delete_all" style='float:right;' >Add Testimonials</a>
        </div>
        <!-- /.card-header --> 
        {!! Form::open(['method'=>'POST', 'url'=> route('admin.testimonial.deleteall'), 'id'=>'myForm']) !!}
        ...
        <div class="card-body"><a class="btn btn-primary btn-sm pull-left delete_all" style='float:right;color:white;' onclick="return deleteAllRecord();">Delete Selected </a>
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
              @if(!empty(count($TestimonialData)))
                @foreach($TestimonialData as $data)
                  <tr>
                    <td><input type="checkbox" name="testimonialid[]" class="sub_chk" value="{{$data->id}}"></td>
                    <td>
                      @if($data->photo)
                    ...
                    <td>{{$data->v_designation}} </td>
                    <td>{!!$data->testimonial_status!!}</td>
                    <td>{{$data->created_at}}</td>
                    <td>
                      <a href="{{route('admin.testimonial.testimonialedit',$data->id)}}" class="btn btn-info btn-sm">Edit</a>  
                      <a href="#;" class="btn btn-danger btn-sm" onclick="deleteRecord('{{route('admin.testimonial.recdelete',$data->id)}}')">Delete</a>
                      @if($data->i_status==1)
                        <a href="#;" onclick="activeRecord('{{route('admin.testimonial.recupdatestatus',[$data->id,0])}}')" class="btn btn-success btn-sm">Inactive</a></td>
                      @else 
